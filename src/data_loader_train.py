...
class HippocampusDataset(Dataset):
    def __init__(self, folder_path, csv_file, target_size=(256, 256), threshold=0.15):
        """
        Dataset pour les slices contenant les hippocampes uniquement.
    ...
    def _get_scan_info(self):
        """
        Extracts information about valid scans (AD and CN only).
        """
        info_list = []
        for _, row in self.csv_file.iterrows():
            scan_id = row[0]  # ID from the CSV file
            status = row[4]   # CN or AD
            
            # Include only AD and CN
            if status not in ["CN", "AD"]:
                continue
            
            # Binary label: 0 for CN, 1 for AD
            class_label = 0 if status == "CN" else 1

            # Scan file path
            scan_file = os.path.join(self.folder_path, f"n_mmni_fADNI_{scan_id}_1.5T_t1w.nii.gz")
            if os.path.exists(scan_file):
                info_list.append((scan_file, class_label))
    ...
    def _generate_slice_indices(self):
        ...
        """
        indices = []
        for scan_idx, (scan_file, _) in enumerate(self.data_info):
    ...
    def __getitem__(self, idx):
        ...
        scan_file, class_label = self.data_info[scan_idx]
